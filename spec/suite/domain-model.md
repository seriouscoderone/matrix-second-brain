...
- `owner` -- enum `alice | bob | shared` -- who owns this record
...
4. **Rule**: Every captured record must have an `owner` (alice, bob, or shared) and a `createdBy` string.
...
- **Owner**: One of `alice`, `bob`, or `shared`. Determines who is responsible for a record. The AI pipeline assigns ownership based on message content and sender.
...
- **Multi-household / multi-tenant support**: The current `owner` enum is hardcoded to `alice | bob | shared`. Can this be extended to support arbitrary household members? This affects the database schema (enum migration), the AI prompt (owner assignment rules), and the config model.
  - *Why it matters*: Limits adoption to exactly two named users.
  - *Who can answer*: Product owner / system architect.
